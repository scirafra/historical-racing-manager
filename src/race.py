import os
import random as rd
from datetime import timedelta

import numpy as np
import pandas as pd

# --------------------------------------
# Model state (DataFrames persisted on disk)
# --------------------------------------
results = pd.DataFrame()
races = pd.DataFrame()
standings = pd.DataFrame()
point_system = pd.DataFrame()
circuits = pd.DataFrame()
circuit_layouts = pd.DataFrame()

# Legacy counters for F1 crash/death statistics
crashes = 0
deaths = 0
f1_races = 0


# --------------------------------------
# Persistence
# --------------------------------------
def load(base_path: str) -> bool:
    """Load race-related DataFrames from disk.

    Expects files:
      - stands.csv
      - races.csv
      - pointSystem.csv
      - results.csv
      - circuits.csv
      - circuit_layouts.csv
    """
    global standings, races, point_system, results, circuits, circuit_layouts

    required = [
        "stands.csv",
        "races.csv",
        "pointSystem.csv",
        "results.csv",
        "circuits.csv",
        "circuit_layouts.csv",
    ]
    for fname in required:
        if not os.path.exists(base_path + fname):
            return False

    standings = pd.read_csv(base_path + "stands.csv")
    races = pd.read_csv(base_path + "races.csv")
    if not races.empty and "race_date" in races.columns:
        races["race_date"] = pd.to_datetime(races["race_date"], errors="coerce")

    point_system = pd.read_csv(base_path + "pointSystem.csv")
    results = pd.read_csv(base_path + "results.csv")
    circuits = pd.read_csv(base_path + "circuits.csv")
    circuit_layouts = pd.read_csv(base_path + "circuit_layouts.csv")
    return True


def save(base_path: str) -> None:
    """Persist race-related DataFrames to disk."""
    if not base_path:
        return
    races.to_csv(base_path + "races.csv", index=False)
    standings.to_csv(base_path + "stands.csv", index=False)
    point_system.to_csv(base_path + "pointSystem.csv", index=False)
    results.to_csv(base_path + "results.csv", index=False)
    circuits.to_csv(base_path + "circuits.csv", index=False)
    circuit_layouts.to_csv(base_path + "circuit_layouts.csv", index=False)


# --------------------------------------
# Queries / Reports (read-only helpers)
# --------------------------------------
def get_results_for_series_and_season(series_id: int, season: int) -> pd.DataFrame:
    """Return results filtered by series and season."""
    df = results[(results["seriesID"] == series_id) & (results["season"] == season)][
        [
            "driverID",
            "teamID",
            "engineID",
            "chassiID",
            "pneuID",
            "raceID",
            "position",
            "round",
        ]
    ].copy()
    return df.reset_index(drop=True)


def pivot_results_by_race(series_id: int, season: int, fill_value=None) -> pd.DataFrame:
    """
    Pivot results into per-race columns.
    - Non-championship races (round == 0) → NC1, NC2, ...
    - Replaces 999 -> Crash, 998 -> Death
    - Adds final_position & final_points if available
    """
    df = get_results_for_series_and_season(series_id, season)
    if df.empty:
        return pd.DataFrame()

    df["round"] = df["round"].fillna(0).astype(int)
    df["position"] = df["position"].replace({999: "Crash", 998: "Death"})

    zero_rids = sorted(df.loc[df["round"] == 0, "raceID"].unique())
    zero_map = {rid: f"NC{i + 1}" for i, rid in enumerate(zero_rids)}

    df["col_label"] = df.apply(
        lambda r: zero_map[r["raceID"]] if r["round"] == 0 else str(r["round"]), axis=1
    )

    pivot = df.pivot(
        index=["driverID", "teamID", "engineID", "chassiID", "pneuID"],
        columns="col_label",
        values="position",
    )

    # keep race order by raceID
    order = df[["col_label", "raceID"]].drop_duplicates().sort_values("raceID")
    labels = order["col_label"].tolist()
    pivot = pivot[labels]

    pivot = pivot.fillna("" if fill_value is None else fill_value)
    pivot.reset_index(inplace=True)
    pivot.columns.name = None

    # merge final standings if there was at least round 1
    if "1" in labels:
        st2 = standings.loc[
            (standings["seriesID"] == series_id)
            & (standings["year"] == season)
            & (standings["typ"] == "driver"),
            ["subjectID", "round", "points", "position"],
        ].copy()
        if not st2.empty:
            st2["round"] = pd.to_numeric(st2["round"], errors="coerce").dropna().astype(int)
            last_idx = st2.groupby("subjectID")["round"].idxmax()
            final = st2.loc[last_idx].rename(
                columns={
                    "subjectID": "driverID",
                    "position": "final_position",
                    "points": "final_points",
                }
            )[["driverID", "final_position", "final_points"]]
            pivot = pivot.merge(final, on="driverID", how="left")

    return pivot


def get_seasons_for_series(series_id: int) -> list[int]:
    """Return sorted list of available seasons for a given series."""
    if results.empty:
        return []
    seasons = results.loc[results["seriesID"] == series_id, "season"].unique()
    return sorted(seasons.tolist())


def all_time_best(drivers_model, series_id: int) -> pd.DataFrame:
    """Return all-time best drivers by final positions in a given series."""
    filtered = standings[(standings["seriesID"] == series_id) & (standings["typ"] == "driver")]
    if filtered.empty:
        return pd.DataFrame()

    # last round of each season
    max_rounds = filtered.groupby("year")["round"].max().reset_index()
    result = pd.merge(filtered, max_rounds, on=["year", "round"])
    result = result.rename(columns={"subjectID": "driverID"})

    # count finishes per position
    position_counts = result.pivot_table(
        index="driverID", columns="position", aggfunc="size", fill_value=0
    )
    sorted_df = position_counts.sort_values(
        by=position_counts.columns.tolist(), ascending=[False] * len(position_counts.columns)
    ).reset_index()

    names = drivers_model.drivers[["driverID", "forename", "surname"]]
    return pd.merge(sorted_df, names, on="driverID", how="left")


# --------------------------------------
# Core simulation
# --------------------------------------
def _simulate_outcome(row: pd.Series) -> str:
    """Random event based on car speed, reliability, and safety.
    Returns one of: 'Good' | 'Crash' | 'Death'
    """
    speed_limit = max(int(row.get("carSpeedAbility", 0)), 0)
    reliability = max(int(row.get("carReliability", 0)), 0)
    safety = max(int(row.get("carSafety", 0)), 0)

    if speed_limit <= 0:
        return "Crash"  # degenerate case

    rnd1 = np.random.randint(0, speed_limit * 1000)
    if rnd1 < reliability:
        rnd2 = np.random.randint(0, speed_limit + 1)
        return "Death" if rnd2 < safety else "Crash"
    return "Good"


def prepare_race(
    drivers_model,
    series_model,
    manufacturer_model,
    contracts_model,
    races_today: pd.DataFrame,
    idx: int,
    current_date,
) -> list[int]:
    """Prepare and simulate a single race at index `idx` in `races_today`.

    Uses drivers_model.active_drivers and contract tables to assemble the grid,
    computes car performance values, picks point rules for season/series,
    and calls the simulation.
    """
    # Active driver-team contracts for the year
    active_dt = contracts_model.DTcontract[
        (contracts_model.DTcontract["active"] == True)
        & (contracts_model.DTcontract["startYear"] <= current_date.year)
        & (contracts_model.DTcontract["endYear"] >= current_date.year)
    ]
    active_dt = active_dt[active_dt["driverID"].isin(drivers_model.active_drivers["driverID"])]

    # Teams allowed in this series
    series_id = int(races_today.iloc[idx]["seriesID"])
    teams_in_series = contracts_model.STcontract[
        contracts_model.STcontract["seriesID"] == series_id
    ]["teamID"]
    grid_dt = active_dt[active_dt["teamID"].isin(teams_in_series)]

    # Join with driver abilities
    selected = pd.merge(
        grid_dt,
        drivers_model.active_drivers[["driverID", "ability"]],
        on="driverID",
        how="left",
    )

    # Initialize car stats columns
    for col in ("power", "reliability", "safety", "e", "c", "p"):
        if col not in selected:
            selected[col] = 0

    # Manufacturer-team contracts and parts for this season
    active_mt = contracts_model.MTcontract[
        (contracts_model.MTcontract["startYear"] <= current_date.year)
        & (contracts_model.MTcontract["endYear"] >= current_date.year)
        & (contracts_model.MTcontract["seriesID"] == series_id)
    ]

    parts = manufacturer_model.car_parts[
        (manufacturer_model.car_parts["seriesID"] == series_id)
        & (manufacturer_model.car_parts["year"] == current_date.year)
    ]

    merged = pd.merge(
        active_mt,
        parts,
        on=["seriesID", "manufacturerID", "partType"],
        how="left",
    )

    # Attach parts and accumulate stats per team (propagated to their drivers)
    for _, part in merged.iterrows():
        team_id = part["teamID"]
        mask = selected["teamID"] == team_id
        # set manufacturer by part type e|c|p
        selected.loc[mask, part["partType"]] = (
            int(part["manufacturerID"]) if pd.notna(part["manufacturerID"]) else 0
        )
        # accumulate stats
        selected.loc[mask, "power"] += int(part.get("power", 0))
        selected.loc[mask, "reliability"] += int(part.get("reliability", 0))
        selected.loc[mask, "safety"] += int(part.get("safety", 0))

    # Track difficulty scaled by wetness
    layout_id = int(races_today.iloc[idx]["layoutID"])
    layout_row = circuit_layouts[circuit_layouts["layoutID"] == layout_id].iloc[0]
    corners = int(layout_row.get("corners", 1) or 1)

    wet = races_today.iloc[idx].get("wet", 1)
    try:
        wet_val = float(wet)
    except Exception:
        wet_val = 1.0
    wet_val = max(wet_val, 1.0)

    track_factor = max(int(corners / wet_val), 1)

    # Build per-driver race input table
    race_data = pd.DataFrame(
        columns=[
            "driverID",
            "ability",
            "carID",
            "carSpeedAbility",
            "carReliability",
            "carSafety",
            "totalAbility",
            "teamID",
            "engineID",
            "chassiID",
            "pneuID",
        ]
    )

    for j in range(len(selected)):
        row = selected.iloc[j]
        power = int(row["power"]) if pd.notna(row["power"]) else 0
        rel = int(row["reliability"]) if pd.notna(row["reliability"]) else 0
        saf = int(row["safety"]) if pd.notna(row["safety"]) else 0
        ability = int(row.get("ability", 0) or 0)

        race_data.loc[len(race_data)] = [
            int(row["driverID"]),
            ability,
            j,
            power,
            int(rel * wet_val),
            int(saf * wet_val),
            int(power * track_factor + ability * 100),
            int(row["teamID"]),
            int(row.get("e", 0) or 0),
            int(row.get("c", 0) or 0),
            int(row.get("p", 0) or 0),
        ]

    race_data = race_data.sort_values(by="totalAbility", ascending=False).reset_index(drop=True)

    # Point rules for this series/season
    rules = series_model.point_rules[
        (series_model.point_rules["seriesID"] == series_id)
        & (series_model.point_rules["startSeason"] <= current_date.year)
        & (series_model.point_rules["endSeason"] >= current_date.year)
    ].reset_index(drop=True)
    ps = point_system[point_system["psID"] == rules.loc[0, "psID"]].reset_index(drop=True)

    # Run the actual simulation
    return simulate_race(
        drivers_model,
        races_today.iloc[idx],
        race_data,
        rules,
        ps,
    )


# Backwards-compatible alias (some older code may call `race(...)`)
def race(drivers_model, race_row, race_data, current_point_rules, ps):
    return simulate_race(drivers_model, race_row, race_data, current_point_rules, ps)


def simulate_race(
    drivers_model,
    race_row: pd.Series,
    race_data: pd.DataFrame,
    current_point_rules: pd.DataFrame,
    ps: pd.DataFrame,
) -> list[int]:
    """Full race simulation. Updates global `results` and `standings`.

    Returns list of driverIDs who died in the race.
    """
    global standings, results, crashes, deaths, f1_races

    died: list[int] = []
    if race_data.empty:
        return died

    # Scale reliability by track safety and wetness
    track_safety = float(race_row.get("trackSafety", 1) or 1)
    wet_val = float(race_row.get("wet", 1) or 1)
    race_data["carReliability"] = (race_data["carReliability"] * track_safety * wet_val).astype(int)

    # Determine outcomes
    race_data["finished"] = race_data.apply(_simulate_outcome, axis=1)

    # F1 crash/death counters
    if int(race_row["seriesID"]) == 1 and int(race_row["season"]) > 1949:
        crashes += int((race_data["finished"] == "Crash").sum())
        deaths += int((race_data["finished"] == "Death").sum())
        f1_races += 1

    finish = race_data[race_data["finished"] == "Good"].reset_index(drop=True)
    crash = race_data[race_data["finished"] == "Crash"].reset_index(drop=True)
    death = race_data[race_data["finished"] == "Death"].reset_index(drop=True)

    # Randomized finishing order among finishers (legacy behavior)
    idx_pool = list(range(len(finish)))
    ranking: list[tuple[int, bool]] = []
    rep_drivers: list[int] = []

    dmax = len(finish)
    for _ in range(dmax):
        chosen = dmax  # sentinel
        while chosen == dmax:
            for j in range(len(idx_pool)):
                if rd.randint(0, 9) < 3:
                    chosen = idx_pool[j]
                    break
        ranking.append((chosen, True))
        rep_drivers.append(int(finish.loc[chosen, "driverID"]))
        idx_pool.remove(chosen)

    # Reputation updates via drivers model
    if hasattr(drivers_model, "race_reputations"):
        drivers_model.race_reputations(int(race_row.get("reputation", 0) or 0), rep_drivers)

    # Determine championship round number
    round_no = 0
    if bool(race_row.get("championship", False)):
        pre = standings[
            (standings["seriesID"] == race_row["seriesID"])
            & (standings["year"] == race_row["season"])
        ]
        round_no = 1 if pre.empty else int(pre["round"].max()) + 1

    # Write results for classified finishers
    for pos, (fin_idx, _) in enumerate(ranking, start=1):
        results.loc[len(results)] = [
            int(race_row["raceID"]),
            int(finish.loc[fin_idx, "driverID"]),
            int(finish.loc[fin_idx, "teamID"]),
            int(finish.loc[fin_idx, "carID"]),
            int(pos),
            int(race_row["season"]),
            int(race_row["seriesID"]),
            int(round_no),
            int(finish.loc[fin_idx, "engineID"]),
            int(finish.loc[fin_idx, "chassiID"]),
            int(finish.loc[fin_idx, "pneuID"]),
        ]

    # Non-finishers: crashes
    for _, row in crash.iterrows():
        results.loc[len(results)] = [
            int(race_row["raceID"]),
            int(row["driverID"]),
            int(row["teamID"]),
            int(row["carID"]),
            999,
            int(race_row["season"]),
            int(race_row["seriesID"]),
            int(round_no),
            int(row["engineID"]),
            int(row["chassiID"]),
            int(row["pneuID"]),
        ]

    # Non-finishers: deaths
    for _, row in death.iterrows():
        results.loc[len(results)] = [
            int(race_row["raceID"]),
            int(row["driverID"]),
            int(row["teamID"]),
            int(row["carID"]),
            998,
            int(race_row["season"]),
            int(race_row["seriesID"]),
            int(round_no),
            int(row["engineID"]),
            int(row["chassiID"]),
            int(row["pneuID"]),
        ]
        died.append(int(row["driverID"]))

    # Update standings if championship event
    if bool(race_row.get("championship", False)):
        _update_standings(
            race_row, race_data, ranking, finish, crash, death, current_point_rules, ps
        )

    return died


def _update_standings(
    race_row: pd.Series,
    race_data: pd.DataFrame,
    ranking: list,
    finish: pd.DataFrame,
    crash: pd.DataFrame,
    death: pd.DataFrame,
    current_point_rules: pd.DataFrame,
    ps: pd.DataFrame,
) -> None:
    """Update global standings after a race."""
    global standings

    pre = standings[
        (standings["seriesID"] == race_row["seriesID"]) & (standings["year"] == race_row["season"])
    ]

    final_blocks = []
    not_finish = pd.concat([crash, death], ignore_index=True)

    for typ in ("driver", "team", "engine", "chassi", "pneu"):
        subj_col = f"{typ}ID"
        subjects = race_data[[subj_col]].drop_duplicates().copy()
        subjects["cars"] = (
            1 if typ == "driver" else (int(current_point_rules.iloc[0].get(f"{typ}Cts", 1)))
        )
        subjects["points"] = 0

        prev_for_typ = pre[pre["typ"] == typ]
        this_round = prev_for_typ["round"].max() if not prev_for_typ.empty else 0
        last_round_block = (
            prev_for_typ[prev_for_typ["round"] == this_round].copy()
            if this_round
            else pd.DataFrame(columns=["subjectID", "points"])
        )

        # Award points to finishers
        for pos, (fin_idx, _) in enumerate(ranking, start=1):
            if fin_idx not in finish.index:
                continue  # skip invalid index
            current_subject = int(finish.loc[fin_idx, subj_col])
            pts = int(ps.iloc[0].get(str(pos), 0))  # use str(pos) to match column names
            mask = (subjects[subj_col] == current_subject) & (subjects["cars"] > 0)
            subjects.loc[mask, ["cars", "points"]] += [-1, pts]

        # Count DNS/DNF entries
        for _, row in not_finish.iterrows():
            current_subject = int(row[subj_col])
            mask = (subjects[subj_col] == current_subject) & (subjects["cars"] > 0)
            subjects.loc[mask, ["cars", "points"]] += [-1, 0]

        # Compose block for this round
        subjects["raceID"] = int(race_row["raceID"])
        subjects["year"] = int(race_row["season"])
        subjects["round"] = 1 if last_round_block.empty else int(this_round) + 1
        subjects["position"] = 0
        subjects["seriesID"] = int(race_row["seriesID"])
        subjects["typ"] = typ

        # Carry previous points
        if not last_round_block.empty:
            prev_pts = last_round_block.set_index("subjectID")["points"]
            subjects["points"] = subjects[subj_col].map(prev_pts).fillna(0).astype(int) + subjects[
                "points"
            ].astype(int)

            # Add missing subjects
            missing = last_round_block[
                ~last_round_block["subjectID"].isin(subjects[subj_col])
            ].copy()
            if not missing.empty:
                missing["round"] = int(subjects["round"].iloc[0])
                missing["raceID"] = int(race_row["raceID"])
                missing = missing.rename(columns={"subjectID": subj_col})
                subjects = pd.concat(
                    [
                        subjects,
                        missing[
                            [subj_col, "points", "raceID", "year", "round", "position", "seriesID"]
                        ].assign(typ=typ),
                    ],
                    ignore_index=True,
                )

        # Sort and assign positions
        subjects = subjects.rename(columns={subj_col: "subjectID"})
        subjects["points"] = subjects["points"].astype(int)
        subjects = subjects.sort_values(
            by=["points", "subjectID"], ascending=[False, True]
        ).reset_index(drop=True)
        subjects["position"] = range(1, len(subjects) + 1)

        # Ensure dtypes
        for col in ["subjectID", "seriesID", "year", "round"]:
            subjects[col] = subjects[col].astype(int)

        final_blocks.append(subjects)

    if final_blocks:
        standings = pd.concat([standings, *final_blocks], ignore_index=True)


# --------------------------------------
# Calendar generation
# --------------------------------------
def plan_races(series_model, current_date) -> None:
    """Generate races for the given year starting from `current_date`.

    - Walks day by day for ~1 season (364 days)
    - Every 5th Sunday schedules races for all active series in that year
    - For the last series on that Sunday, every 6th slot is non‑championship
    """
    global races

    week_counter = 0
    date = pd.Timestamp(current_date)

    for _ in range(364):
        if date.strftime("%a") == "Sun":
            week_counter += 1
            if week_counter % 5 == 0:
                active_series = series_model.series[
                    (series_model.series["startYear"] <= date.year)
                    & (series_model.series["endYear"] >= date.year)
                ]
                for si, srow in active_series.iterrows():
                    new_race_id = 0 if races.empty else int(races["raceID"].max()) + 1

                    # every 6th slot for the LAST series that day becomes non‑championship
                    championship = not (si == active_series.index[-1] and week_counter % 6 == 0)

                    # random circuit & layout
                    if circuits.empty or circuit_layouts.empty:
                        continue
                    track_id = int(rd.choice(circuits["circuitID"].tolist()))
                    matching = circuit_layouts[circuit_layouts["circuitID"] == track_id]
                    if matching.empty:
                        continue
                    layout_id = int(rd.choice(matching["layoutID"].tolist()))
                    safety = float(
                        circuit_layouts.loc[
                            circuit_layouts["layoutID"] == layout_id, "safety"
                        ].iloc[0]
                    )

                    # wetness factor
                    wet_roll = rd.randint(1, 8)
                    wet = rd.randint(1, 50) / 100 + 1 if wet_roll == 8 else 1

                    races.loc[len(races)] = {
                        "raceID": new_race_id,
                        "seriesID": int(srow["seriesID"]),
                        "season": int(date.year),
                        "trackID": track_id,
                        "layoutID": layout_id,
                        "trackSafety": safety,
                        "race_date": date,
                        "name": f"Preteky {srow['name']}",
                        "championship": championship,
                        "reputation": (
                            1000 // int(srow["reputation"]) if int(srow["reputation"]) else 0
                        ),
                        "reward": (
                            1000000 // int(srow["reputation"]) if int(srow["reputation"]) else 0
                        ),
                        "wet": wet,
                    }
        date += timedelta(days=1)
